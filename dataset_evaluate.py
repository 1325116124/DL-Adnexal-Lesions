...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        evaluate_res = self.get_text_evaluate(index)
        ...
        return torch.tensor(np.array([img1.numpy(), img2.numpy(), img3.numpy(),img4.numpy(),img5.numpy(),img6.numpy()])),img7, img8, text_embed, evaluate_res, label, name

    def __len__(self):
        return len(self.images1)

    def get_text_evaluate(self, index):
        # 医生的判断
        name = self.images1[index].split("/")[5]
        # 读取CSV文件
        ...
        res = []
        res.append(filtered_data["高1"].values[0])
        res.append(filtered_data["高2"].values[0])
        res.append(filtered_data["低1"].values[0])
        res.append(filtered_data["低2"].values[0])
        return res
