...
class CrossAttention(nn.Module):
    def __init__(self, feature_dim, dropout=0.7):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        attention_output = torch.bmm(attention_probs, values)

        # 加入残差连接和层归一化
        attention_output += original_queries  # 残差连接
        attention_output = F.layer_norm(attention_output, attention_output.size()[1:])  # 层归一化
...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
        ...
        self.attn = CrossAttention(1)
        self.attn2 = CrossAttention(1)
        self.relu = nn.ReLU()
    ...
    def forward(self, input, input2, input3, no_reshape=False):
        if not no_reshape:
        ...
        if self.reshape:
            ...
            # input2 = self.attn(input2, input1, input1)
            # text = text.unsqueeze(1)
            # x1 = self.relu(torch.mul(base_out, text))
            # text = text.squeeze(1)

            # input2 = self.relu(torch.mul(input2, text))
            # input3 = self.relu(torch.mul(input3, text))

            # temp = torch.cat((input2.unsqueeze(1),input3.unsqueeze(1)), 1)
            
            # base_out = self.attn2(base_out, input2.unsqueeze(1), input2.unsqueeze(1))
            ...
            # input2 = input2.unsqueeze(1).permute(0,2,1)
            # input3 = input3.unsqueeze(1).permute(0,2,1)

            # output = output.permute(0,2,1)
            # output = self.attn(output, input2, input2)
            # output = self.attn2(output, input3, input3)

            # output = output.permute(0,2,1)

            return output.squeeze(1), base_out
